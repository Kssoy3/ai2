...
# Google Drive 파일 ID
file_id = '1jHXj4FxaWipy7NdP3R2ehNEn765C9pZz'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/LgnjcVn/high.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://youtu.be/PCOR7QxFN6k?si=3u73HrcEThRcmay9", start_time=0)
